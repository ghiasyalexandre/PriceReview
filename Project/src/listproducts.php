 ...
 `review`.`customer_id`, count(rating) AS number_rating, count(review_id) AS number_reviews
   FROM
    ...
    ON
     ...
     $sql = " SELECT `product`.`product_id`, `product`.`product_name`, `product`.`product_descrip`, `product`.`product_price`,  `product`.`product_image`, `review`.`customer_id`, count(rating) AS number_rating, count(review_id) AS number_reviews
        FROM
            (`product`
        JOIN `review`)
     WHERE
        ((`product`.`product_id` = `review`.`product_id`) AND (`review`.`customer_id` = '" . $searchItem . "')) 
        GROUP by product_id ORDER BY number_rating DESC;";
